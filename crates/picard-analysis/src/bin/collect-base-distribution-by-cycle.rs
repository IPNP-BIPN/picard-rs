//! `CollectBaseDistributionByCycle` as a runnable binary: the covering array's port side.
//!
//! The mirror image of `MeanQualityByCycle`: metric rows and no histogram at all. One row per
//! cycle per read end, holding the percentage of each base at that cycle, and `PCT_N` meaning
//! "not one of the four bases" rather than literally `N`.
//!
//! What the arguments decide is which reads are counted: `PF_READS_ONLY` drops vendor-failed
//! reads and `ALIGNED_READS_ONLY` drops unmapped ones, and the array's fixtures make both
//! observable.
//!
...
use picard_analysis::cycle::CollectBaseDistributionByCycle;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut collector = CollectBaseDistributionByCycle::default();
    ...
    for metric in collector.finish() {
