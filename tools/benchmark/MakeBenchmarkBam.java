 ...
 * qualities, strands, pairing states, indels, splices, soft and hard clips, no-calls and
 * mismatches, so the collector takes every branch rather than the cheapest one. A benchmark over
 * a million identical perfect reads measures branch prediction, not the tool.
 */
...
public class MakeBenchmarkBam {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Coordinate-sorted, so the writer's own sort is not part of what is measured later.
        ...
        try (SAMFileWriter w = new SAMFileWriterFactory().setCreateIndex(false)
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (rng.nextInt(100) < 50) {
                    ...
                    if (rng.nextInt(100) < 5) flags |= 0x8;
                    if (rng.nextInt(100) < 50) flags |= 0x20;
                }
                if (rng.nextInt(100) < 50) flags |= 0x10;
                if (rng.nextInt(1000) < 5) flags |= 0x200;
                if (rng.nextInt(1000) < 10) flags |= 0x800;
                if (rng.nextInt(1000) < 5) flags |= 0x100;
                r.setFlags(flags);
